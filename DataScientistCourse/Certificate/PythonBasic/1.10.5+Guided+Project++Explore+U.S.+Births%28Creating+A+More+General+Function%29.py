...
# ### 1.10.05 创建一个更加通用的函数(Creating A More General Function)
...
def calc_counts(data, column):
    ...
    for lst in data:
        ...
        dkey = lst[column-1] # 观察了xls表格，星期几在第四列，所以index为3
        births = lst[4]
        
        if dkey in dictionary:
            dictionary[dkey] = dictionary[dkey] + births
        else:
            dictionary[dkey] = births
    return dictionary
...
cdc_year_births = calc_counts(cdc_list,1) #year ,年在xls表格中在第一列
cdc_year_births
