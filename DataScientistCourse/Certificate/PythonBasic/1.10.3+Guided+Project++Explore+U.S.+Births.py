...
def month_births(list_of_lists):
    ...
    for lst in list_of_lists:
        month.append(lst[1])   #挨着排的取list中的月份添加到month 列表中。循环一次取一个
        births.append(lst[4])  #挨着排的取list中的出生数量添加到births 列表中。循环一次取一个
        if month in births_per_month: #判断取出的一个month是否已在births_per_month中作为key了
            births_per_month[month] = births_per_month[month]+ births #yes，则将births的数量加上原先的值，更新赋值
        else:
