...
'''
# 这节讲 延续上一节，做简单统计。每个国家在1989年的酒饮料消费量。
...
#找到原因了：不是我的问题。countries系统已经定义好了。我又定义了一遍countries = world_alcohol[:,2]，有点多次一举了。
...
'''
# 这节讲 找到最能喝的国家。
# 国家x和饮酒量max的组合。这里系统已经给出字典total，包含国家名作为key/饮酒量为value的键值对。
#用print(totals)就可以看到。
'''
...
# 至于hkey在字典的for循环中为什么是指国家，应该是python定义字典的key作为for的关键变量吧？
for hkey in totals:
